/**
 * gemini-key-manager.js — Pool de Chaves Groq com Rotação Automática
 * (exportado com nomes compatíveis para não quebrar imports existentes)
 *
...
class GroqKeyManager {
  ...
  // Alias para compatibilidade com código legado que usa getKey()
...
// ── Singleton global (exportado como geminiKeyManager para compatibilidade) ────
export const geminiKeyManager = new GroqKeyManager();

// ── triggerGroqCooldown: mantido para compatibilidade (cooldown agora é interno) ─
export function triggerGroqCooldown() {
  // Cooldown tratado automaticamente dentro de callGeminiWithRotation
}
...
// ── callGeminiWithRotation — chama Groq com rotação de chaves ─────────────────
/**
 * @param {Function} callFn       - async (apiKey: string) => string
 * @param {string}   agentName
 * @param {number}   maxAttempts
 * @param {object}   _fallback    - mantido na assinatura por compat (não usado)
 */
export async function callGeminiWithRotation(callFn, agentName = 'Agent', maxAttempts = 18, _fallback = {}) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    let key = geminiKeyManager.getNextKey();

    // Todas as chaves em cooldown → aguarda a mais próxima
    if (!key) {
      const waitMs = geminiKeyManager.timeUntilNextKey() + 500;
      if (waitMs > 0 && waitMs < 120_000) {
        console.warn(`  [${agentName}] ⏳ Todas as chaves em cooldown — aguardando ${Math.ceil(waitMs / 1000)}s`);
        await sleep(waitMs);
        key = geminiKeyManager.getNextKey();
      }
      if (!key) {
        throw Object.assign(
    ...
    try {
      ...
      if (status === 429) {
        ...
        geminiKeyManager.reportRateLimit(key, retryAfter);
      ...
      if (status === 401) {
        ...
        geminiKeyManager.reportRateLimit(key, 3600);
        if (attempt < maxAttempts) continue;
      }
...
if (process.argv[1]?.endsWith('gemini-key-manager.js')) {
  ...
  console.log(`  Rotações: ${geminiKeyManager.state.rotations || 0}\n`);
}
