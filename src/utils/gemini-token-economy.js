/**
 * gemini-token-economy.js — Economia de Tokens Gemini (otimização interna)
 *
 ...
 *  3. GATE      — Se Quant confidence ≥ SKIP_THRESHOLD, pula Gemini e usa Quant direto
...
const SKIP_THRESHOLD   = 92;                   // Quant confidence ≥ 92% → skip Gemini (mercado muito claro)
...
// ── Compressão de matchData (sharp agents) ────────────────────────────────────
/**
 * Comprime matchData para reduzir tokens enviados ao Gemini.
 * Aplica a mesma lógica do BaseMarketAgent mas para o payload dos sharp agents.
...
export function compactMatchData(matchData) {
  ...
  // Extrai forma recente — últimas 5 partidas (mais relevante, menos tokens)
  const forma5 = (str) => (str || '').slice(-5) || undefined;

  // Compacta time (home/away)
  ...
  const compact = clean({
    ...
    // Campos de contexto live (quando disponíveis)
...
// ── Gate de confiança — skip Gemini quando Quant já é muito seguro ────────────
/**
 * Retorna true se o Gemini pode ser pulado para economizar tokens.
 * Usado quando o modelo Quant já tem confiança muito alta (≥92%) em mercados
 * calibrados, onde a contribuição do Gemini é marginal (<3pp na decisão final).
 *
 * NUNCA pula em mercados não calibrados ou com confiança abaixo do threshold.
 */
export function shouldSkipGemini(market, quantConfidence, quantProbability) {
  if (!SKIP_MARKETS.has(market)) return false;
  if (!quantConfidence || quantConfidence < SKIP_THRESHOLD) return false;
  // Só pula se a probabilidade também é alta (evita falso positivo por confiança)
...
// ── Tracking de uso ────────────────────────────────────────────────────────────
const _session = {
  calls:      0,  // chamadas reais ao Gemini
  cacheHits:  0,  // chamadas evitadas por cache
  gateSkips:  0,  // chamadas evitadas por gate de confiança
